...
def softmax_loss(Z, y):
    ...
    exp_Z = np.exp(Z)
    log_sum_exp_Z = np.log(np.sum(exp_Z, axis=1))
...
def softmax_regression_epoch(X, y, theta, lr = 0.1, batch=100):
    ...
    for i in range(0, num_examples, batch):
        ...
        e_y = np.zeros((batch_size, num_classes))
        e_y[np.arange(batch_size), y_batch] = 1
        grid = X_batch.T @ (z - e_y) / batch_size
        theta -= lr * grid
...
def nn_epoch(X, y, W1, W2, lr = 0.1, batch=100):
    ...
    for i in range(0, num_examples, batch):
        X_batch = X[i:i + batch]
        y_batch = y[i:i + batch]
        ReLU_w1x = np.maximum(X_batch @ W1, 0)
        Z1 = ReLU_w1x
        G2 = np.exp(Z1 @ W2) / np.sum(np.exp(Z1 @ W2), axis=1, keepdims=True)
        batch_size = y_batch.shape[0]
        I_y = np.zeros_like(G2)
        I_y[np.arange(batch_size), y_batch] = 1
        G2 = G2 - I_y
        G1 = G2 @ W2.T * (Z1 > 0)
        grid_w1 = X_batch.T @ G1 / batch_size
        grid_w2 = Z1.T @ G2 / batch_size
        W1 -= lr * grid_w1
        W2 -= lr * grid_w2
    ### 你的代码结束
